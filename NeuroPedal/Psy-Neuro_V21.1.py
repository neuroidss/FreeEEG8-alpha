...
# Конфигурация для OpenBCI Cyton (как в v14)
...
# ==========================================
# 1. DEEP HIERARCHICAL PSY-CORTEX
# ==========================================
class TemporalHierarchyColumn(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.l1 = nn.Conv1d(in_channels, 32, kernel_size=3, padding=1, dilation=1)
        self.l2 = nn.Conv1d(32, 64, kernel_size=3, padding=2, dilation=2)
        self.l3 = nn.Conv1d(64, out_channels, kernel_size=3, padding=4, dilation=4)
        self.act = nn.ELU()
...
class DeepSpatiotemporalBrain(nn.Module):
    def __init__(self):
        ...
        self.col_eeg = TemporalHierarchyColumn(N_EEG, 64)
        self.col_motor = TemporalHierarchyColumn(N_MOTOR, 64)
        self.col_audio = TemporalHierarchyColumn(N_AUDIO, 64)
        self.col_thalamus = nn.Sequential(nn.Conv1d(N_THALAMUS, 32, 1), nn.ELU())
        
        self.association_area = nn.GRU(64 + 64 + 64 + 32, 256, num_layers=3, batch_first=True, dropout=0.1)
        
        self.action_generator = nn.Sequential(
            nn.Linear(256, 256), nn.ELU(), 
            nn.Linear(256, 128), nn.ELU(),
            nn.Linear(128, N_MOTOR), nn.Sigmoid()
        )
        self.forward_model = nn.Sequential(
            nn.Linear(256 + N_MOTOR, 128), nn.ELU(), 
            nn.Linear(128, N_AUDIO), nn.Sigmoid()
        )
    ...
    def forward(self, eeg, motor, audio, thalamus):
        ...
        next_action = self.action_generator(state)
        
        efference_copy = torch.cat([state, next_action], dim=1)
...
def get_thalamus_clocks(t_len):
    # ИСПРАВЛЕНИЕ: Добавили 'global', чтобы функция видела счетчик
    global global_frame_counter
    t = np.arange(global_frame_counter, global_frame_counter + t_len)
    macro_freq = t / (30.0 * 4.0); meso_freq  = t / 30.0; micro_freq = t / 7.5
    clocks = np.stack([
        np.sin(2 * np.pi * macro_freq), np.cos(2 * np.pi * macro_freq),
        np.sin(2 * np.pi * meso_freq),  np.cos(2 * np.pi * meso_freq),
        np.sin(2 * np.pi * micro_freq), np.cos(2 * np.pi * micro_freq)
    ], axis=1)
...
class Win(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Deep Psy-Cortex V21.1: Live EEG")
        ...
        self.p_rhythm = self.plot_w.addPlot(title="Superhuman Patterns (Yellow: Real Hands | Blue: Virtual Brain)")
    ...
    def upd(self):
        ...
        self.lbl.setText(f"LIVE DJ MODE | Loss: {shared_loss:.4f} | RTX Batch: {int(dyn_batch)}")
