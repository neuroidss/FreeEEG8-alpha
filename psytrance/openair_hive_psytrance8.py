...
# Твоя точная геометрия (не трогаем)
RADIUS = 10.0
...
# ==========================================
# 2. АУДИО ДВИЖОК — ФИНАЛЬНАЯ ВЕРСИЯ
# ==========================================
class HiveAudioEngine:
    def __init__(self):
        ...
        # ===================== НАСТРОЙКИ ГРОМКОСТИ =====================
        self.master_gain   = 0.0018      # ← твоё комфортное значение
        self.kick_gain     = 1.40
        self.bass_gain     = 1.15
        self.gamma_gain    = 0.48        # сильно приглушён (высокие)
        self.lead_gain     = 0.58
        self.hat_gain      = 0.65

    def process_audio(self, outdata, frames, time_info, status):
        outdata.fill(0)
        dt = 1.0 / AUDIO_FS
        ...
        for i in range(MAX_DEVICES):
            ...
            if self.timer_kick[i] < 0.38:
                ...
                kick = np.sin(2*np.pi*(48*T - 3.5*np.exp(-55*T))) * np.exp(-16*T) * np.clip(u['delta_amp']*2.2, 0.6, 1.6)
                outdata[:,0] += kick * self.kick_gain
                self.timer_kick[i] += frames*dt

            # BASS
            if self.timer_bass[i] < 0.48:
                T = self.timer_bass[i] + t_arr
                bass = 2*((T%1)-0.5) * np.exp(-13*T) * np.clip(u['theta_amp']*1.9, 0.35, 1.0)
                bass = np.tanh(bass*(2.2 + u['theta_amp']*5))
                outdata[:,0] += bass * self.bass_gain
                self.timer_bass[i] += frames*dt

            # SPATIAL THETA-GAMMA (рабочая память по кругу)
            if g > 0.14:
                ...
                voice = np.sin(2*np.pi*pc) * (0.7 + 0.8*np.sin(2*np.pi*pc*3.7))
                voice *= g * 1.65 * np.exp(-((t*3.8)%1)*11)
                outdata[:,1] += voice * self.gamma_gain
                self.gamma_angle_mem[i] = angle

            # ALPHA LEAD
            lead_vol = np.clip((a - 0.06)*6, 0, 0.72)
            if lead_vol > 0.01:
                c_freq = 150 + u['lead_x'] * 360
                ...
                lead = np.sin(2*np.pi*pc + 3.5*np.sin(2*np.pi*pm))
                outdata[:,1] += lead * lead_vol * self.lead_gain * np.exp(-((t*2)%1)*7)

        # === ФИНАЛЬНОЕ СВЕДЕНИЕ ===
        mix = outdata[:,0] + outdata[:,1]
...
# ==========================================
# 4. BLE + GPU DSP (исправленная ciPLV + instant phase)
# ==========================================
class SingleDeviceManager:
    ...
    async def apply_stage_settings(self, stage):
        ...
        try:
            ...
            await self.write_and_verify(0x03, sps << 2, 0x001C, "SPS")   # ← ИСПРАВЛЕНО
...
class GpuDspWorker(QThread):
    ...
    def get_instant_phase(self, sig, low, high):
        N = sig.shape[-1]
        fft_data = torch.fft.fft(sig)
        freqs = torch.fft.fftfreq(N, 1/EEG_FS, device=DEVICE)
        mask = (freqs >= low) & (freqs <= high)
        an = torch.zeros_like(fft_data)
        an[mask] = fft_data[mask] * 2.0
        an_sig = torch.fft.ifft(an)
        return torch.angle(an_sig[-1]).item()

    def get_amp(self, sig, low, high):
        N = sig.shape[-1]
        fft_data = torch.fft.fft(sig)
        freqs = torch.fft.fftfreq(N, 1/EEG_FS, device=DEVICE)
        mask = (freqs >= low) & (freqs <= high)
        an = torch.zeros_like(fft_data)
        an[mask] = fft_data[mask] * 2.0
        return torch.mean(torch.abs(torch.fft.ifft(an))[-125:]).item()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OPEN-AIR HIVE PSYTRANCE — Phase Triggered (multi-brain)")
        ...
        self.stream = sd.OutputStream(channels=2, samplerate=AUDIO_FS,
    ...
    def update_ui(self, ui_data):
        ...
        if active_count > 0:
            ...
            self.lbl_global.setText(f"BPM: {bpm:.0f}   |   {active_count} ЧЕЛОВЕК В ХАЙВЕ   |   PHASE PSYTRANCE")
        else:
            self.lbl_global.setText("BPM: —   |   0 ЧЕЛОВЕК В ХАЙВЕ   |   PHASE PSYTRANCE")
