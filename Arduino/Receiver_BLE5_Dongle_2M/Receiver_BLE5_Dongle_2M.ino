...
class ClientCallbacks : public NimBLEClientCallbacks {
    void onConnect(NimBLEClient* pClient) override { 
        ...
        // ЗАПРАШИВАЕМ ПЕРЕКЛЮЧЕНИЕ НА 2 MBPS (Ускоряет передачу, снижает шум эфира)
...
void setup() {
    // 921600 бод — отлично справляется с потоком данных
    Serial.begin(921600);
    ...
    // Ставим такую же мощность на приемнике для надежности пакетов подтверждения (ACK)
...
void loop() {
    if (doConnect) {
        ...
        // Удаляем объект после отработки
        delete targetDevice;
